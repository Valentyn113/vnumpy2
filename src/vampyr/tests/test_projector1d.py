...
def test_ScalingProjector_scalar_func():
    """Scalar 1D functions (lambda x: ...) should now work with projectors."""
    def f(x):
        return x
...
def test_ScalingProjector_vector_func():
    """Vector-style 1D functions (lambda r: r[0]...) still work (backward compat)."""
    def f(r):
        return r[0]
