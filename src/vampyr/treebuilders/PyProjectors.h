...
template <int D> class PyScalingProjectorComplex final {
public:
    PyScalingProjectorComplex(const MultiResolutionAnalysis<D> &mra, double prec)
            : min_scale(mra.getRootScale())
            , precision(prec)
            , MRA(mra) {}
    PyScalingProjectorComplex(const MultiResolutionAnalysis<D> &mra, int scale)
            : min_scale(scale)
            , precision(-1.0)
            , MRA(mra) {
        if (this->min_scale < this->MRA.getRootScale()) MSG_ERROR("Invalid scale");
    ...
    std::unique_ptr<FunctionTree<D, ComplexDouble>> operator()(std::function<ComplexDouble(const Coord<D> &r)> func) {
        auto out = std::make_unique<FunctionTree<D, ComplexDouble>>(this->MRA);
        if (this->precision > 0.0) {
            // With the adaptive projection we want s+w repr at finest scale
            project<D>(this->precision, *out, func);
        } else {
            // With the fixed scale projection we want pure s repr at finest scale
            int depth = this->min_scale - this->MRA.getRootScale();
            build_grid<D, ComplexDouble>(*out, depth);
            project<D>(-1.0, *out, func);
...
template <int D> class PyWaveletProjectorComplex final {
public:
    PyWaveletProjectorComplex(const MultiResolutionAnalysis<D> &mra, int scale)
            : min_scale(scale)
            , MRA(mra) {
        if (this->min_scale < this->MRA.getRootScale()) MSG_ERROR("Invalid scale");
    }
